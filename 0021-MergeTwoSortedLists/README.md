...
This is the first linked list problem in the repo — a linked list is a chain of individually-allocated **nodes**, each holding a value (`.val`) and a pointer to the next node (`.next`), rather than a contiguous block like an array/Python list. You can't jump to a random position; you can only get anywhere by following `.next` pointers one step at a time from the head.
...
4. Return `dummy.next` (skipping over the placeholder itself, which was never part of the real result).
...
- The dummy-node pattern is a common trick for building a new linked list from scratch: without it, there's nothing to attach the very first real node onto (`None.next = ...` would crash), so a throwaway placeholder node solves that bootstrapping problem cleanly.
- `tail.next = list1 if list1 else list2` is a compact way of handling the leftover-list step: whichever list still has nodes remaining (is truthy, i.e. not `None`) gets attached wholesale.
